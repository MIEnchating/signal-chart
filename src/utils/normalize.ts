...
export function generateVisualMapFromSeries(
  ...
  series.forEach((s, index) => {
    ...
    // 根据 gridIndex 计算位置
    // 使用 bottom 定位，确保色卡不超出容器
    let bottomPosition: number = 50  // 默认距离底部 50px

    // 如果有多个 grid，根据 gridIndex 调整位置
    if (gridIndex === 0) {
      bottomPosition = 350  // 上半部分：距离底部 350px（约在上半部分中心）
    } else if (gridIndex === 1) {
      bottomPosition = 50   // 下半部分：距离底部 50px（约在下半部分中心）
    }
    ...
    const autoVisualMap: VisualMapOption = {
      ...
      orient: 'vertical',
      right: 20,
      bottom: bottomPosition,  // 使用 bottom 定位
      itemWidth: 20,
      itemHeight: 200,
      splitNumber: 5,
      textStyle: {
      ...
      containLabel: true  // 默认包含标签，确保标签不超出容器
    }
